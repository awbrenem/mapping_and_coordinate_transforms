...
for j = 0, nrec-1 do begin
    ...
    lshells[j] = (txp^2+typ^2+tzp^2)^1.5/(txp^2+typ^2)
    ilats[j] = acos(1d/sqrt(lshell))*deg
endfor
store_data, pre0+'ilat', uts, ilats, $
    limits = {ytitle:'ILat!C(deg)'}
store_data, pre0+'lshell', uts, lshells, $
        limits = {ytitle:'Lshell!C(deg)'}
...
for j = 0, n_elements(uts)-1 do begin
    ...
    lshells08[j] = (txf^2+tyf^2+tzf^2)^1.5/(txf^2+tyf^2)
...
store_data, pre0+'lshell08', uts, lshells08, $
    limits = {ytitle:'Lshell08!C(deg)'}
