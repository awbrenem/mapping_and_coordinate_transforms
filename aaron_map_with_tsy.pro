...
;start_time --> For T04, string date and time of the start of the storm (e.g. '2016-01-20/19:44:00')
;                For other models, set to start time of when you want mapping to begin
;duration --> For T04 this is the duration of the storm (seconds)
...
;   the Earth's surface (PROBABLY WANT TO MAKE THIS ~50 KM ABOVE SURFACE, FOR EXAMPLE).
...
pro aaron_map_with_tsy,model,start_time,duration,sc,gsm_coord,Kpval,R0=R0,rlim=rlim,set_tilt=tiltvar
  ...
  if model ne 't89' and model ne 'none' then begin
...
;  tinterpol_mxn,gsm_coord,newtimes,newname=gsm_coord
  ...
  in_coord='gsm',newname=sc+'_out_eq_foot_gsm',/km,R0=R0,rlim=rlim,get_tilt=tilt_dat,set_tilt=tiltvar

  print,'********END TRACE: eq foot GSM'

  ttrace2iono,gsm_coord,external_model=model,par=modpars,$
  in_coord='gsm',newname=sc+'_out_iono_foot_north_gsm',/km,R0=R0,rlim=rlim,get_tilt=tilt_dat,set_tilt=tiltvar


  print,'********END TRACE: iono foot north GSM'

  ttrace2iono,gsm_coord,external_model=model,par=modpars,$
  in_coord='gsm',newname=sc+'_out_iono_foot_south_gsm',/km,/south,R0=R0,rlim=rlim,set_tilt=tiltvar

  print,'********END TRACE: iono foot south GSM'
